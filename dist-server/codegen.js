...
function buildConnections(roomId, elements) {
    const roomMap = new Map(elements.filter(e => e._type === 'Room').map(r => [r.id, r]));
    return elements
        .filter(e => e._type === 'Connector' &&
...
export function generateInform7(rawMap) {
    const elements = rawMap.elements ?? [];
    ...
    Handlebars.registerHelper('isStartRoom', (room) => room.isStart === true);
